...
class HuggingFaceBeaconAI:
    ...
    def recommend_for_user(self, user_id, top_n=5, filter_liked=True, liked_event_ids=None, batch_size=1024):
        """Generate recommendations using pre-trained model weights - optimized for inference"""
        if self.model is None:
        ...
        # Filter liked items - much simpler approach using direct event IDs
        liked_item_internal_ids = set()
        if filter_liked and liked_event_ids:
            liked_item_internal_ids = {
                self.item_id_map[event_id] 
                for event_id in liked_event_ids 
                if event_id in self.item_id_map
            }
        
        # Get recommendations
        recommendations = []
        for idx in np.argsort(-scores):
            if not filter_liked or idx not in liked_item_internal_ids:
    ...
    def load_model_if_exists(self):
        """Load model without requiring feature recomputation - for inference only"""
        try:
            return self.load_trained_model()
    ...
    def is_model_loaded(self):
        ...
        return (self.model is not None and 
                hasattr(self, 'user_id_map') and 
                hasattr(self, 'item_id_map') and
                hasattr(self, 'user_feature_tensor') and 
                hasattr(self, 'item_feature_tensor'))
